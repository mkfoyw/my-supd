...
# -------------------------------------------------
export PRJ_ROOT=`pwd`
export PRJ_NAME="supd-miner"
export GOBIN=$PRJ_ROOT/bin
export GO111MODULE=on

# 设定sup [command] all 所遍历的目录
export BUILD_ALL_PATH="
$PRJ_ROOT/tools/supd/cmd/supd
$PRJ_ROOT/tools/upgrade 
$PRJ_ROOT/tools/manager
$PRJ_ROOT/tools/butler
"
...
export PUB_ROOT_RES="apps etc bin script install.sh" # 根目录下需要打包的文件夹列表，如"etc"等, 空格字符串表示不使用
export PUB_APP_RES=" " # app下的文件夹列表，如"res public"等, 空格字符串表示不使用
